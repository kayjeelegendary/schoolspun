...
.directive('sirAccordion',['$compile','$timeout', function($compile,$timeout){
  ...
  return {
    ...
    template: template,
    controller: ('sirAccordionCtrl',['$scope','$http',function ($scope,$http) {
      $scope.config = {
		...
		contentIdnr: $scope.config.contentIdnr || ''
		
		
      };
	 console.log($scope.config.contentIdnr+ "hery");
	    
		    $scope.name = 'kkkkkniocek';
 ...
 var rt = "heyhey";
          item = item + '<div  class="' + scope.config.bottomContentClass + '"><div><a href="#/correcter/' + collection[currentIndex].title2 + '/' + collection[currentIndex].IDnr + '"> '+  collection[currentIndex].bottomContent +' </a>' +'<div>' + rt+'</div>' + '</div>' 
          + '</div></div></div></div></div>';
        }
        return item + itemRegen(collection, parentIndex, currentIndex + 1, level);
      };
// i got stuck at sendnig a form from this stage on
//this was going to help me mark and correct answers from the same page
